...
// PubKeySessionIndex maps a remote public key to its session ID.
// This is persisted so the in-memory sessionsByPubKey map can be
// rebuilt on startup.
type PubKeySessionIndex struct {
	state           protoimpl.MessageState `protogen:"open.v1"`
	SessionId       string                 `protobuf:"bytes,1,opt,name=session_id,json=sessionId,proto3" json:"session_id,omitempty"`
	RemotePublicKey []byte                 `protobuf:"bytes,2,opt,name=remote_public_key,json=remotePublicKey,proto3" json:"remote_public_key,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *PubKeySessionIndex) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *PubKeySessionIndex) GetRemotePublicKey() []byte {
	if x != nil {
		return x.RemotePublicKey
	}
	return nil
...
const file_box_proto_rawDesc = "" +
	...
	"\x14resume_recv_sequence\x18\x04 \x01(\x04R\x12resumeRecvSequence\"_\n" +
	"\x12PubKeySessionIndex\x12\x1d\n" +
	"\n" +
	"session_id\x18\x01 \x01(\tR\tsessionId\x12*\n" +
	"\x11remote_public_key\x18\x02 \x01(\fR\x0fremotePublicKey\"\xc0\x01\n" +
