...
Initiator                                   Responder
    ...
    |                                            |
    |  ---- SignedTransport[REQUEST_HS] ------>  |   (§6.3, the prefix+
    |        Handshake {                         |    suffix concatenation
    |          SessionKey: <full sessionID>      |    step is skipped; both
    |        }                                   |    sides send the full
    |                                            |    sessionID instead of
    |  <--- SignedTransport[ACCEPT_HS] --------  |    generating fresh random
    |        Handshake {                         |    halves)
...
The only structural difference from §6.3 is that `SessionKey` carries the full
predetermined session ID on both sides instead of each side generating a random
half, so the `sessionID` matches the one being resumed. The initiator solves
for `sessionID = opts.sessionID` directly and skips the `prefix + suffix`
concatenation step.

### 7.1 Resumption Initiator Constraint

Resumption is always initiated by the dialer. The server never dials — it
accepts incoming `ResumeRequest` messages but never sends one. This means:

- If the **dialer** disconnects, it re-dials with `DialWithResume` and
  resumption proceeds normally.
- If the **server** disconnects, it simply waits for the dialer to reconnect
  and attempt resumption. The server does not initiate a new connection.
- **Role reversal** (the original server dialing the original client) does not
  occur in practice — dialer and server roles are fixed for the lifetime of
  the application and do not swap between sessions.

This asymmetry matches the existing protocol architecture: the `Dialer` holds
`DialWithResume(sessionID)` and the `Server` holds `handleResume`, with no
`ServerWithResume` equivalent because the server never needs to initiate. Server
can disable session resumption via the `ServeWithResumeEnabled(false)` option.
...
- **Resumption root derivation.** `Transport.setResumptionRoot` derives the
  resumption root from the MLKEM768 shared secret and session ID. The root is
  stored internally and never exposed.
...
- **Initiator flow.** `Dialer.attemptResume` sends `ResumeRequest`, receives
  `ResumeAccept`, and proceeds directly to the resumed handshake — no
  application intervention required.
- **Session ID reuse.** `handshakeOpts.sessionID` and `DialWithResume()` allow
  the dialer to reuse a prior session ID transparently. The handshake skips
  the prefix+suffix concatenation and uses the full predetermined ID.
- **Resumption control.** `ServeWithResumeEnabled(false)` disables resumption
  on the server — incoming `ResumeRequest` messages are treated as an
  unexpected route, forcing a full Introduction from the dialer. Enabled by
  default.

### Application layer handles

- **Resumption decision.** The application chooses whether to call
  `DialWithResume` (if a session record exists for the target peer) or dial
  fresh with a cold Introduction.
- **Handler.** The application's `HandlerFunc` receives resumed transports the
  same as cold ones; no distinction is needed at the handler level.
- **Error handling.** The application interprets `ErrResumptionRejected` and
  decides whether to retry with a cold Introduction.
